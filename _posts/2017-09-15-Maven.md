...
- 의존성 관리(Dependency management): 이 기능은 Maven 설정 파일의 <dependencyManagement/>엘리멘트에 의존 관계에 있는 라이브러리와 버전을 명시적으로 정의한다.<br>
...
컴파일폴더는 삭제되고 항상 다시 컴파일된 파일들이 생기지만 배포폴더의 경우 컴파일폴더의 파일들이 copy되는 것이라 <b>새로 생긴 파일들은 반영이 되지만 사라진 파일의 경우 반영되지않는다.</b><br>
예를들면 아래의 그림과 같다.
<table>
<tr>
<td>적용 전 빌드결과물</td>
